...
Try<Owned<Discovery>> Discovery::create(const Flags& flags)
{
  ...
  if (!creators.contains(flags.docker_discovery)) {
    return Error("Unknown or unsupported strategy '" +
                 flags.docker_discovery + "'");
  }

  return creators[flags.docker_discovery](flags);
}
...
{
  return path::join("file:///", flags.docker_discovery_local_dir, name);
}

...
#ifndef __MESOS_DOCKER_REGISTRY_TOKEN_MANAGER_HPP__
#define __MESOS_DOCKER_REGISTRY_TOKEN_MANAGER_HPP__
...
struct Token
{
   ...
   */
  static Try<Token> create(const std::string&);
...
private:
  ...
  static Result<process::Time> getTimeValue(const JSON::Object& object,
...
private:
  /**
   * Path at the authorization server that handles the token request.
   */
  const static std::string TOKEN_PATH_PREFIX;
  ...
  TokenManager(const process::http::URL& realm,
      process::Owned<TokenManagerProcess>&);
...
class TokenManagerProcess : public process::Process<TokenManagerProcess>
{
public:
  static Try<process::Owned<TokenManagerProcess>> create();

  process::Future<Token> getToken(const process::http::URL&);
...
#endif // __MESOS_DOCKER_REGISTRY_TOKEN_MANAGER_HPP__
